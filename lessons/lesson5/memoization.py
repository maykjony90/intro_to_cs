...
# Also we need to check execution time of the function:
import time
...
# ADVICE: You should learn how to think in a opposite way, so that you
# can start to take adventage of simplicity of the 'not equal to' expression.
# You can see the difference just by comparing above and below code...
...
# find n'th element of fibonacci numbers:
def cached_fibo(n):
    if n == 1 or n == 0:
        return n
    else:
...
print('result of fibo proc(n\'th number of fibonacci numbers): ')
print(cached_execution(cache, cached_fibo,5))
